...
def point3(v,z,dmax,dmin,theta): #theta must be in radians
    x=0.0 #starting x, set location to 0    
    vx= v*cos(theta)   #calculating starting velocity component X
    vz= v*sin(theta)   #calculating starting velocity component Z
    ...
    for i in range(len(xpoints)):
            if xpoints[i] < dmax and xpoints[i]> dmin and zpoints[i] > 3.038 and zpoints[i] < 3.058:
                plt.plot(xpoints,zpoints) #only points that go through hoop will be plotted
                plt.axis('equal')
                plt.xlabel("X")
                plt.ylabel("Z")
                break
